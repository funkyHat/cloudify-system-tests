...
class DockerComputeTestCase(TestCase):
    ...
    def setUp(self):
        ...
        def cleanup():
            with self.manager_env_fabric(warn_only=True) as api:
                api.sudo('docker rm -f $(docker ps -aq)')
    ...
    def kill_container(self, node_id):
        container_id = self.client.node_instances.list(
            node_id=node_id)[0].runtime_properties['container_id']
        with self.manager_env_fabric() as api:
            api.sudo('docker rm -f {0}'.format(container_id))
